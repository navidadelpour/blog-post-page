...
class simpleRating{
  ...
  init(){
    var html='<div class="simple-rating star-rating float-left mb-3">';
    for(var i=0;i<5;i++){html+='<i class="material-icons mi-star empty" data-rating="'+(i+1)+'">star</i>';}
    html+='</div>';
  ...
  setstars(rating){
    ...
    for(var i=0;i<5;i++){
      ...
      if(i<rating){
        $(starObj).removeClass('empty');
      }else{
        $(starObj).addClass('empty');
      }
    }
  }
}